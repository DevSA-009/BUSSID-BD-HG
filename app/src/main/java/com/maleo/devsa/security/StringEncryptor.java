...
/**
 * StringEncryptor — Same Hex+Octal algorithm as original StringEDCSA.
    ...
    public static String decrypt(String encrypted) {
        ...
        try {
            ...
            while (idx < oct.length()) {
                if (idx + 3 <= oct.length()) {
                    try { second.append((char) Integer.parseInt(oct.substring(idx, idx + 3), 8)); idx += 3; continue; }
                    catch (NumberFormatException ignored) {}
                }
                if (idx + 2 <= oct.length()) {
                    second.append((char) Integer.parseInt(oct.substring(idx, idx + 2), 8)); idx += 2;
                } else break;
            }
