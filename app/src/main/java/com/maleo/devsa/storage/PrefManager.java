...
public class PrefManager {
    ...
    // Activation Key
    public void saveActivationKey(String key) { prefs.edit().putString(KEY_ACTIVATION_KEY, key).apply(); }
    public String getActivationKey()          { return prefs.getString(KEY_ACTIVATION_KEY, null); }
    public boolean isActivated()              { return getActivationKey() != null; }
    public void clearActivationKey()          { prefs.edit().remove(KEY_ACTIVATION_KEY).apply(); }

    // OBB patch version
    public void saveObbPatchVersion(String v) { prefs.edit().putString(KEY_OBB_PATCH_VER, v).apply(); }
    public String getObbPatchVersion()        { return prefs.getString(KEY_OBB_PATCH_VER, "unknown"); }
    ...
    // DLC
    public void setDlcInstalled(boolean v)  { prefs.edit().putBoolean(KEY_DLC_INSTALLED, v).apply(); }
    public boolean isDlcInstalled()         { return prefs.getBoolean(KEY_DLC_INSTALLED, false); }

    // Report dialog — shown once per interval
    public void saveReportShownNow() { prefs.edit().putLong(KEY_REPORT_LAST, System.currentTimeMillis()).apply(); }
